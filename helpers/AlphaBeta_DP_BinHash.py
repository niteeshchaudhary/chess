...
class AlphaBeta_DP_BinHash:
        ...
        if isMaxplayer:
            ...
            for child in moves:
                ...
                for i in range(8):
                    for j in range(8):
                        if next_board[i][j]:
                            dp_key[i*8+j]=1
                dp_key_int= dp_key.dot(1 << np.arange(dp_key.size,dtype=np.uint64)[::-1])
            ...
            for child in moves:
                ...
                if dp_key_int in self.mindic.keys():
                    ...
                    self.mindic[dp_key_int]=myscore
                if myscore<minscore:
                    best_move=child
                    minscore=myscore
    ...
    def getNextMove(self,board,game_obj,player="black",depth=6):
        count=0
        for i in range(8):
            for j in range(8):
                if board[i][j]:
        ...
        if self.count!=count:
            ...
            self.mindic={}
            self.count=count
        move,_=self.minmax(board,game_obj,player,-10000000,1000000,depth,True)
        print(move)
        return move